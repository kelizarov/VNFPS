...
void AVNPlayerController::SetupInputComponent() {
  ...
  InputComponent->BindAxis("LookUpRate", this,
                           &AVNPlayerController::LookUpRate);
  ...
  if (Val != 0.f) {
    if (FMath::Abs(Val) > 0.8f) {
      if (CameraSpeed < BaseLookUpRate * AccelerationFactor)
        CameraSpeed += AccelerationFriction * FMath::Abs(Val) *
    ...
    GEngine->AddOnScreenDebugMessage(
        0, 0.001f, FColor::Blue,
        FString::Printf(TEXT("Val : %f"), CameraSpeed));
...
void AVNPlayerController::TurnRate(float Val) {
  ...
  if (Val != 0.f) {
    ...
    GEngine->AddOnScreenDebugMessage(
        1, 0.001f, FColor::Red, FString::Printf(TEXT("Val : %f"), CameraSpeed));
