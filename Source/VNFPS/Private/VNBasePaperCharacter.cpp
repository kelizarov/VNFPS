...
AVNBasePaperCharacter::AVNBasePaperCharacter(
    const class FObjectInitializer &ObjectInitializer)
    : Super(ObjectInitializer
                .SetDefaultSubobjectClass<UVNCharacterMovementComponent>(
...
float AVNBasePaperCharacter::GetHealth() const { return Health; }
...
void AVNBasePaperCharacter::SetCharacterDirection(
    ECharacterDirection NewDirection) {
...
bool AVNBasePaperCharacter::IsAlive() const { return Health > 0; }

// Refactor with hash table
void AVNBasePaperCharacter::DetermineDirection() {
  ...
  int32 Side = static_cast<int32>(
      FMath::RadiansToDegrees(FMath::Acos(FVector::DotProduct(
          RightVector, DirectionToPlayer.GetSafeNormal()))) /
      30);
...
void AVNBasePaperCharacter::OnChangedDirection() {
  if (!bIsDying) {
    if (!bIsPlayingAnimation)
      ...
      PlayAnimation(CurrentAnimation, MySprite->IsLooping(),
                    MySprite->GetPlaybackPosition());
  }
}
...
void AVNBasePaperCharacter::PlayAnimation(
    TMap<ECharacterDirection, UPaperFlipbook *> *AnimationToPlay,
    bool bIsLooping, float PlaybackPosition) {
  ...
  if (!bIsLooping) {
    ...
    GetWorldTimerManager().SetTimer(TimerHandle_FinishedAnimation, this,
                                    &AVNBasePaperCharacter::OnFinishedAnimation,
                                    SpriteToPlay->GetTotalDuration(), false);
  }
  MySprite->Play();
}

void AVNBasePaperCharacter::PlayAnimation(UPaperFlipbook *AnimationToPlay,
                                          bool bIsLooping,
  ...
  if (!bIsLooping) {
    ...
    GetWorldTimerManager().SetTimer(TimerHandle_FinishedAnimation, this,
                                    &AVNBasePaperCharacter::OnFinishedAnimation,
                                    AnimationToPlay->GetTotalDuration(), false);
  }
  MySprite->Play();
}
...
void AVNBasePaperCharacter::DetermineAnimationState() {
  ...
  bIsMoving = FMath::Abs(GetVelocity().Size()) > 0;
}
...
float AVNBasePaperCharacter::TakeDamage(float Damage,
  ...
  if (ActualDamage > 0.f) {
    ...
    if (Health <= 0) {
      ...
      if (DamageEvent.DamageTypeClass) {
        // USDamageType* DmgType =
        // Cast<USDamageType>(DamageEvent.DamageTypeClass->GetDefaultObject());
...
bool AVNBasePaperCharacter::CanDie(float KillingDamage,
...
bool AVNBasePaperCharacter::Die(float KillingDamage,
                                FDamageEvent const &DamageEvent,
  ...
  // AController *KilledPlayer = Cast<AController>(GetOwner());
  // GetWorld()->GetAuthGameMode<AVNGameMode>()->Killed(Killer, KilledPlayer,
  // this, DamageType);

  OnDeath(KillingDamage, DamageEvent, Killer ? Killer->GetPawn() : NULL,
          DamageCauser);
  return true;
}

void AVNBasePaperCharacter::OnDeath(float KillingDamage,
                                    FDamageEvent const &DamageEvent,
...
void AVNBasePaperCharacter::PlayHit(float DamageTaken,
                                    struct FDamageEvent const &DamageEvent,
                                    APawn *PawnInstigator, AActor *DamageCauser,
                                    bool bKilled) {
  if (bKilled && DeathSound) {
    PlayDeathAnimation();
    UGameplayStatics::SpawnSoundAttached(DeathSound, RootComponent, NAME_None,
                                         GetActorLocation(),
                                         EAttachLocation::SnapToTarget, true);
  } else if (HitSound) {
    PlayStaggerAnimation();
    UGameplayStatics::SpawnSoundAttached(HitSound, RootComponent, NAME_None,
                                         GetActorLocation(),
                                         EAttachLocation::SnapToTarget, true);
  }
}
...
FRotator AVNBasePaperCharacter::GetAimOffsets() const {
  ...
  const FVector AimDirLS =
      ActorToWorld().InverseTransformVectorNoScale(AimDirWS);
...
void AVNBasePaperCharacter::GetHit(float Damage) {
  TakeDamage(Damage, FDamageEvent(NULL), GetWorld()->GetFirstPlayerController(),
             this);
}
...
bool AVNBasePaperCharacter::IsSprinting() const {
  ...
  return (bWantsToRun || bWantsToRunToggle) && !IsTargeting() &&
         !GetVelocity().IsZero() &&
         (FVector::DotProduct(GetVelocity().GetSafeNormal2D(),
                              GetActorRotation().Vector()) > 0.1);
}
