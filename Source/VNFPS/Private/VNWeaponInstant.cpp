...
AVNWeaponInstant::AVNWeaponInstant(
...
void AVNWeaponInstant::FireWeapon() {
  ...
  const float ConeHalfAngleHorizontal =
      FMath::DegreesToRadians(CurrentSpreadHorizontal * 0.5f);
  const float ConeHalfAngleVertical =
      FMath::DegreesToRadians(CurrentSpreadVertical * 0.5f);
  const FVector AimDir = GetAdjustedAim();
  const FVector ShootDir = WeaponStream.VRandCone(
      AimDir, ConeHalfAngleHorizontal, ConeHalfAngleVertical);
...
void AVNWeaponInstant::DealDamage(const FHitResult &Impact,
  ...
  Impact.GetActor()->TakeDamage(PointDmg.Damage, PointDmg, MyPawn->Controller,
                                this);
}
