...
AVNProjectile::AVNProjectile(const class FObjectInitializer &ObjectInitializer)
  ...
  CollisionComp = ObjectInitializer.CreateDefaultSubobject<USphereComponent>(
      this, TEXT("Collision"));
...
void AVNProjectile::Explode(const FHitResult &Impact) {
  const FVector NudgeImpactLocation =
      Impact.ImpactPoint + Impact.ImpactNormal * 10.f;

  if (RadialDamage > 0.f && RadialRadius > 0.f) {
    UGameplayStatics::ApplyRadialDamage(
        this, RadialDamage, NudgeImpactLocation, RadialRadius, DamageType,
