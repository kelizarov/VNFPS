...
void AVNWeapon::PostInitializeComponents()
{
	...
	TimeBetweenShots = FiringAnimation != nullptr ? FiringAnimation->GetTotalDuration() : 60.0f / ShotsPerMinute;
	CurrentAmmo = 0;
	CurrentAmmoInClip = MaxAmmoPerClip;
	ReloadDuration = ReloadAnimation != nullptr ? ReloadAnimation->GetTotalDuration() : NoAnimReloadDuration;
}
...
void AVNWeapon::OnEquip(bool bPlayAnimation)
{
	...
	if (bPlayAnimation)
	{
		...
		GetWorldTimerManager().SetTimer(TimerHandle_EquipFinished, this, &AVNWeapon::OnEquipFinished, Duration, false);
	}
	else
	{
		OnEquipFinished();
...
bool AVNWeapon::IsEquipped() const
{
	return bIsEquipped;
}
...
bool AVNWeapon::CanFire() const
{
	...
	bool StateOK = CurrentState == EWeaponState::Idle || CurrentState == EWeaponState::Firing;
...
{
	const AVNPlayerController* PC = MyPawn ? Cast<AVNPlayerController>(MyPawn->Controller) : nullptr;
...
void AVNWeapon::HandleFiring()
{
	...
	if (MyPawn)
	{
		...
		{
			GetWorldTimerManager().SetTimer(TimerHandle_HandleFiring, this, &AVNWeapon::HandleFiring, TimeBetweenShots, false);
		}
	}
...
void AVNWeapon::SimulateWeaponFire()
{
	...
	GetWorldTimerManager().SetTimer(TimerHandle_StopSimulatingFire, this, &AVNWeapon::StopSimulatingWeaponFire, FiringAnimation->GetTotalDuration(), false);
}
...
EWeaponState AVNWeapon::GetCurrentState() const
{
	return CurrentState;
}
...
void AVNWeapon::OnBurstStarted()
{
	...
	{
		GetWorldTimerManager().SetTimer(TimerHandle_HandleFiring, this, &AVNWeapon::HandleFiring, LastFireTime + TimeBetweenShots - GameTime, false);
	}
	else
	{
		HandleFiring();
...
float AVNWeapon::GetEquipStartedTime() const
{
	return EquipStartedTime;
}

float AVNWeapon::GetEquipDuration() const
{
...
int32 AVNWeapon::GetCurrentAmmoInClip() const
{
...
int32 AVNWeapon::GetMaxAmmoPerClip() const
{
...
void AVNWeapon::StartReload()
{
	if (CanReload())
	{
		...
		else
		{
			...
			GetWorldTimerManager().SetTimer(TimerHandle_ReloadWeapon, this, &AVNWeapon::ReloadWeapon, FMath::Max(0.1f, ReloadDuration - 0.1f), false);
			GetWorldTimerManager().SetTimer(TimerHandle_StopReload, this, &AVNWeapon::StopSimulateReload, ReloadDuration, false);
		}
	}
}
...
void AVNWeapon::ReloadWeapon()
{
	...
	{
		int32 ClipDelta = FMath::Min(GetMaxAmmoPerClip() - GetCurrentAmmoInClip(), GetCurrentAmmo() - GetCurrentAmmoInClip());
...
bool AVNWeapon::CanReload() const
{
	...
	bool bHasAmmo = (GetCurrentAmmoInClip() < GetMaxAmmoPerClip()) && ((GetCurrentAmmo() - GetCurrentAmmoInClip()) > 0);
	bool bStateOK = ((CurrentState == EWeaponState::Idle) || (CurrentState == EWeaponState::Firing));

	return bCanReload && bHasAmmo && bStateOK;
}

float AVNWeapon::PlayAnimation(UPaperFlipbook* AnimationToPlay, bool bIsLooping, float PlaybackPosition)
{
