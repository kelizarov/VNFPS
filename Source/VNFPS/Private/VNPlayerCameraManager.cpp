...
void AVNPlayerCameraManager::BeginPlay()
{
	Super::BeginPlay();
}
...
void AVNPlayerCameraManager::UpdateCamera(float DeltaTime)
{
	...
	if (MyPawn)
	{
		if (MyPawn->bIsCrouched && !bWasCrouched && MyPawn->GetCharacterMovement()->MovementMode != EMovementMode::MOVE_Falling)
		{
			CurrentCrouchOffset = MaxCrouchZOffset;
		}
		else if (!MyPawn->bIsCrouched && bWasCrouched && MyPawn->GetCharacterMovement()->MovementMode != EMovementMode::MOVE_Falling)
		{
			CurrentCrouchOffset -= MaxCrouchZOffset;
		}
		bWasCrouched = MyPawn->bIsCrouched;

		CurrentCrouchOffset = FMath::Lerp(CurrentCrouchOffset, 0.f, FMath::Clamp(CrouchLerpVelocity * DeltaTime, 0.f, 1.0f));
		FVector CurrentCameraPosition = MyPawn->GetCamera()->GetRelativeTransform().GetLocation();
		FVector NewCameraPosition = FVector(CurrentCameraPosition.X, CurrentCameraPosition.Y, DefaultCameraZOffset + CurrentCrouchOffset);
